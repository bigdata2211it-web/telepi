...
export function resolvePathFromCwd(filePath, cwd = process.cwd()) {
    const expandedPath = expandHomePath(filePath);
    return path.isAbsolute(expandedPath) ? expandedPath : path.resolve(cwd, expandedPath);
}
export function getDefaultTelePiConfigPath(homeDirectory = getHomeDirectory()) {
 ...
 */
export function getDefaultSystemdUserDir(homeDirectory = getHomeDirectory()) {
 ...
 */
export function getDefaultLogDir(homeDirectory = getHomeDirectory(), platform) {
    if (platform === "darwin") {
