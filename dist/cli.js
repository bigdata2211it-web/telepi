...
export function ensureNoArguments(command, args) {
    if (args.length > 0) {
...
export async function runSetupCommand(args) {
    ...
    if (platform === "linux" && result.context.serviceUnitPath) {
        console.log(`Service: ${result.context.serviceUnitPath} (${result.unitUpdated ? "updated" : "unchanged"})`);
    }
    else if (platform === "darwin" && result.context.launchAgentPath) {
        console.log(`${serviceLabel}: ${result.context.launchAgentPath} (${result.unitUpdated ? "updated" : "unchanged"})`);
    }
    console.log(`Extension: ${result.context.extensionDestinationPath} (${result.extensionInstalledAs})`);
...
export function runStatusCommand() {
    ...
    const extensionSummary = status.extension.mode === "symlink" && status.extension.targetPath
