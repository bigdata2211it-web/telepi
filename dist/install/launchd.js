...
export function buildLaunchAgentPlist(context) {
    ...
    return replacements.reduce((content, [placeholder, value]) => content.replace(placeholder, value), template);
}
export function writeLaunchAgentPlist(context) {
...
export function reconcileLaunchAgent(context) {
    const actions = [];
...
export function getLaunchAgentStatus(context) {
...
export function readLaunchAgentPlist(context) {
    if (!context.launchAgentPath || !existsSync(context.launchAgentPath)) {
        return undefined;
    }
    return readFileSync(context.launchAgentPath, "utf8");
}
export function readLaunchAgentWorkingDirectory(plistContents) {
    return readLaunchAgentStringValue(plistContents, "WorkingDirectory");
}
export function readLaunchAgentEnvironmentVariables(plistContents) {
    const environmentBlock = plistContents.match(/<key>EnvironmentVariables<\/key>\s*<dict>([\s\S]*?)<\/dict>/)?.[1];
    if (!environmentBlock) {
        return {};
    }
    const values = {};
...
export function getInstalledConfigStatus(context) {
...
function buildEnvironmentVariablesBlock(context) {
    const entries = [["TELEPI_CONFIG", context.configPath]];
...
function readLaunchAgentStringValue(plistContents, key) {
...
function matchValue(input, pattern) {
