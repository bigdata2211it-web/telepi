...
import { TELEPI_EXTENSION_FILENAME, TELEPI_LAUNCH_AGENT_FILENAME, TELEPI_LAUNCHD_LABEL, TELEPI_SERVICE_UNIT_FILENAME, TELEPI_SYSTEMD_LOG_DIR_RELATIVE, TELEPI_SYSTEMD_USER_DIR_RELATIVE, } from "./shared.js";
...
export function detectPlatform() {
    ...
    if (process.platform === "linux")
        return "linux";
    throw new Error(`telepi setup is only supported on macOS and Linux. Current platform: ${process.platform}`);
}
/** Resolve the full install context for the current platform. */
export function resolveTelePiInstallContext(cliModuleUrl) {
 ...
 */
export function getServiceManager(platform) {
    if (platform === "darwin")
 ...
 */
export function getPlatformInstallHint(packageName) {
    if (process.platform === "darwin") {
...
function sanitizePath(rawPath) {
    ...
    const clean = entries.filter((entry) => {
        ...
        if (trimmed.includes(" "))
            return false;
        if (trimmed.includes(":"))
            return false; // "bin":Unknown
        if (trimmed.startsWith("npm"))
            return false; // npm error output
        if (trimmed.startsWith("To see"))
            return false;
        return true;
    }).join(":");
...
// Internal helpers
// ---------------------------------------------------------------------------
function resolveInstalledCliEntrypointPath(packageRoot, raw) {
    if (path.extname(raw) !== ".ts")
        return raw;
