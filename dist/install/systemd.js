...
export function writeSystemdUnit(context) {
    return writeUnitFile(context);
}
function buildUnitFile(context) {
...
function writeUnitFile(context) {
...
function getStatus(context) {
    ...
    const result = spawnSync("systemctl", ["--user", "show", "telepi.service", "--property=ActiveState,MainPID"], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
    if (result.error) {
        return {
            unitExists,
            plistExists: false,
...
// --- internal helpers ---
function runSystemctl(actions, args) {
...
function formatSystemdValue(value) {
    if (!/[\s"\\]/.test(value)) {
        return value;
    }
