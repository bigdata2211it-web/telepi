...
export async function copyToClipboard(text) {
    ...
    if (process.platform === "linux") {
        ...
        if (copyViaCommand("wl-copy", [], text))
            return true;
        if (copyViaCommand("xclip", ["-selection", "clipboard"], text))
            return true;
        if (copyViaCommand("xsel", ["--clipboard"], text))
            return true;
        return false;
    }
    return false;
...
function copyViaCommand(command, args, text) {
    try {
