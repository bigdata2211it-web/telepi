...
import { buildLaunchAgentPlist, getInstalledConfigStatus, } from "./install/launchd.js";
import { resolveTelePiInstallContext, getServiceManager } from "./install/platform.js";
import {} from "./install/shared.js";
...
// ---- getTelePiStatus ----
export function getTelePiStatus(cliModuleUrl) {
    ...
    const serviceStatus = serviceMgr.getStatus(context);
    return {
...
// ---- setupTelePi ----
export async function setupTelePi(cliModuleUrl, options = {}) {
    const context = resolveTelePiInstallContext(cliModuleUrl);
...
// ---- internal helpers ----
function ensureInstallInputsExist(context) {
    if (path.extname(context.cliEntrypointPath) === ".ts") {
        throw new Error(`telepi setup requires a built CLI entrypoint at ${path.join(context.packageRoot, "dist", "cli.js")}. Run \`npm run build\` and rerun \`telepi setup\`.`);
    }
    const requiredPaths = [
