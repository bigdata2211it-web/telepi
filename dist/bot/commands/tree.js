...
import { renderBranchConfirmation, renderLabels, renderTree, } from "../../tree.js";
import { stripHtml } from "../message-rendering.js";
export function createTreeCommandHandlers(deps) {
    const { getContextKey, getExistingSession, isBusy, pendingTreeNavs, pendingBranchButtons, clearPendingTreeView, setPendingTreeView, buildTreeKeyboard, buildKeyboard, safeReply, } = deps;
    const collectLabelsMap = (piSession) => {
    ...
    const handleTreeCommand = async (ctx, target, commandText) => {
    ...
    const handleBranchCommand = async (ctx, target, commandText) => {
        const contextKey = getContextKey(target);
        const piSession = getExistingSession(target);
        if (isBusy(target)) {
    ...
    const handleLabelCommand = async (ctx, target, commandText) => {
        const piSession = getExistingSession(target);
        if (isBusy(target)) {
        ...
        if (parts.length >= 2) {
            ...
            if (entry) {
                ...
                await safeReply(ctx, `🏷️ Label <b>${escapeHTML(labelName)}</b> set on <code>${escapeHTML(maybeId)}</code>`, {
        ...
        await safeReply(ctx, `🏷️ Label <b>${escapeHTML(args)}</b> set on current leaf <code>${escapeHTML(leafId)}</code>`, {
