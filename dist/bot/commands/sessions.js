...
export function createSessionCommandHandlers(deps) {
    const { getContextKey, getOrCreateSession, getExistingSession, isBusy, beginSwitching, endSwitching, buildKeyboard, clearContextPickers, clearContextPromptMemory, refreshChatScopedCommands, syncChatScopedCommands, setChatCommandSignature, removeSession, pendingSessionPicks, pendingSessionButtons, pendingWorkspacePicks, pendingWorkspaceButtons, safeReply, surfaceStartupErrorDiagnostics, } = deps;
    const handleSessionsCommand = async (ctx, target, commandText) => {
        ...
        const orderedPicks = [];
        const sessionButtons = allSessions.map((session, idx) => {
    ...
    const handleNewCommand = async (ctx, target) => {
        const contextKey = getContextKey(target);
        if (isBusy(target)) {
    ...
    const handleHandbackCommand = async (ctx, target) => {
        const contextKey = getContextKey(target);
